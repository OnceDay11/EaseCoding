 ...
 *  (7) 支持可定位性, 支持 Debug 日志.
 */
struct easeds_array {
    const char *name;         /* 数组名称, 预留字段, 可用于调试和日志输出 */
 ...
 * easeds_array_resize          调整数组容量, 成功返回0, 失败返回-1
...
// 调整数组容量, 成功返回0, 失败返回-1
int32_t easeds_array_resize(struct easeds_array *array, uint32_t new_capacity);
